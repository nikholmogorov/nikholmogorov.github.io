...
describe('Calc Component', () => {
    ...
    test('returns 0 when amount is set to 0', () => {
        render(
            <MemoryRouter>
                <Calc {...mockProps} />
            </MemoryRouter>
    ...
    test('handles clearing input when value becomes null', () => {
        render(
            <MemoryRouter>
                <Calc {...mockProps} />
            </MemoryRouter>
    ...
    test('renders 0 when rate for target currency is missing', () => {
        const ratesWithoutTarget: Rate[] = [
            { date: '2026-01-01', base: 'RUB', quote: 'USD', rate: 0.01 },
        ]

        render(
            <MemoryRouter>
                <Calc {...mockProps} rates={ratesWithoutTarget} />
            </MemoryRouter>
        )
    ...
    test('handles sparse rates array with undefined or null elements safely', () => {
        const sparseRates = [
            null,
            { date: '2026-01-01', base: 'RUB', quote: 'USD', rate: 0.01 },
        ] as unknown as Rate[]
        ...
        render(
            <MemoryRouter>
                <Calc {...mockProps} rates={sparseRates} />
            </MemoryRouter>
        )
        expect(screen.getByText('1.00')).toBeInTheDocument()
    })
